...
export default function Navbar({section, changeSection}) {
  ...
  const DrawerList = (
    <Box sx={{ width: 'auto' }} role="presentation" onClick={toggleDrawer(false)} className='flex flex-col h-screen'>
      <List >
        {['Overview','Why Us', 'Getting Started', 'Schedule', 'Contact Us'].map((text) => (
          <ListItem key={text} disablePadding onClick={()=>{changeSection(text)}}>
            <ListItemButton>
              <ListItemIcon >
                {text === 'Overview' && <img src={Earth} alt="Earth" className='h-7 sm:h-10 lg:h-12 xl:h-13 lg:mr-5'/>}
                {text === 'Why Us' && <img src={Question} alt="Profile" className='h-7 sm:h-10 lg:h-12 xl:h-13 lg:mr-5'/>}
                ...
                {text === 'Become A Tutor' && <img src={People} alt="Message" className='h-7 sm:h-10 lg:h-12 xl:h-13 lg:mr-5'/>}
                {text === 'Contact Us' && <img src={Message} alt="Message" className='h-7 sm:h-10 lg:h-12 xl:h-13 lg:mr-5'/>}
              </ListItemIcon>
      ...
      <Divider />
      <List>
        {['Plans and Pricing','Become a Tutor','Join The Community', 'Login'].map((text, index) => (
          <ListItem key={text} disablePadding onClick={()=>{changeSection(text)}}>
            <ListItemButton>
              <ListItemIcon sx={{maxWidth: '10px'}}>
                ...
                {text === 'Login' && <img src={Login} alt="Login" className='h-4 sm:h-5 lg:h-7 xl:h-7'/>}
                {text === 'Plans and Pricing' && <img src={Money} alt="Login" className='h-4 sm:h-5 lg:h-7 xl:h-7'/>}
              </ListItemIcon>
              <ListItemText primary={
                <Typography
                    sx={{
