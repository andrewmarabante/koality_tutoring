...
export default function PlansPricing() {
    ...
    function submit(){
        ...
        if(showSubmitPlan[0] === 'The Great Koala'){membership = 'great'}
        if(showSubmitPlan[0] === 'The Good Koala'){membership = 'good'}
        if(showSubmitPlan[0] === 'Regular Koala'){membership = 'reg'}
        if(showSubmitPlan[0] === 'KoALa'){membership = 'koala'}

        const data = {
            membership: membership
        }

            fetch( server +'student/subscribe' , {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
                    ...
                    <div className='h-full w-full px-5'>
                        ...
                        <AnimatePresence>
                            ...
                            >
                                ...
                                >
                                    {plans && currentUser && plans.map((plan, i) => {

                                        if (
                                            (currentUser.membership === 'great' && plan.title !== 'The Great Koala') ||
                                            (currentUser.membership === 'good' && plan.title !== 'The Good Koala') ||
                                            (currentUser.membership === 'reg' && plan.title !== 'Regular Koala') ||
                                            (currentUser.membership === 'koala' && plan.title !== 'KoALa')
                                        ) { return }
                                                        ...
                                                        <CardContent sx={{ height: 200 }}>
                                                            ...
                                                            <div className='flex flex-col justify-between h-full'>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    {currentUser && !currentUser.membership ? <div>
                                                                        {!plan.available && <Button variant="text" color='warning' fullWidth>SOLD OUT</Button>}
                                                                        {plan.available && !plan.lastFew && <Button variant="contained" fullWidth onClick={() => setShowSubmitPlan([plan.title, plan.price])}>Subscribe</Button>}
                                                                        {plan.available && plan.lastFew && <Button variant="contained" color='secondary' fullWidth onClick={() => setShowSubmitPlan([plan.title, plan.price])}>last few remaining</Button>}
                                                                    </div> :
                                                                        <Button variant="text" color='warning'>Current Plan</Button>
                                                                    }
